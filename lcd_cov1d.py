...
def build_conv1d_model():
    ...
    model.add(layers.Conv1D(32,11,padding='valid',input_shape=(train_X.shape[1],1)))
    model.add(layers.BatchNormalization())
    model.add(layers.Activation('relu'))
    model.add(layers.MaxPooling1D(2,padding='valid'))
    model.add(layers.Conv1D(16,7,padding='valid'))
    model.add(layers.BatchNormalization())
    model.add(layers.Activation('relu'))
    model.add(layers.MaxPooling1D(2,padding='valid'))
...
f = open('conv_model.json', 'w')
f.write(json_string)
f.close()
model.save_weights('conv_weights.h5')
...
testA_Y = model.predict(testA_X, batch_size=batch_size)
testB_Y = model.predict(testB_X, batch_size=batch_size)
...
resultA.to_csv('resultA.csv', header=False,index=False)
resultB = pd.DataFrame({'ID':raw_testB['ID'].values,'Y':testB_Y.ravel()})
resultB.to_csv('resultB.csv', header=False,index=False)
